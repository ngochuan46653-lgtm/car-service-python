...
class ServecesService:
    @staticmethod
    def get_all():
        return RepositoriesManagerServeces.list_all_display_rows()

    @staticmethod
    def add(data):
        name = str(data.get("name", "")).strip()
        price = int(str(data.get("price", "0")).replace(",", "").strip() or 0)
        RepositoriesManagerServeces.insert(name, price)

    @staticmethod
    def update(service_id, data):
        name = str(data.get("name", "")).strip()
        price = int(str(data.get("price", "0")).replace(",", "").strip() or 0)
        RepositoriesManagerServeces.update(service_id, name, price)

    @staticmethod
    def delete(service_id):
        RepositoriesManagerServeces.delete(service_id)
